...
export function parseReviewInput(body: unknown): ReviewInput {
  ...
  const siblingName =
    typeof b.siblingName === "string" ? b.siblingName.trim() : "";
  if (!siblingName) {
    throw new GenerateReviewError(
      "siblingName is required",
      400,
      "bad_request",
    );
  }
...
export async function handleGenerateReview(
  ...
  try {
    ...
    if (error instanceof GenerateReviewError && error.status === 400)
      throw error;
...
export default async function handler(
  ...
  try {
    ...
    const code =
      error instanceof GenerateReviewError ? error.code : "generation_failed";
    const message =
